...
void ArbiterSingleForwardHelper::UpdateForwardingState(int64_t t) {
    ...
    if (fstate_file) {
        ...
        while (getline(fstate_file, line)) {
            ...
            int64_t path_id = parse_positive_int64(comma_split[5]);  // 新增：path_id
            ...
            // === 修改：根据是否地面站 + path_id 决定调用哪个函数 ===
            Ptr<ArbiterSingleForward> arbiter = m_arbiters.at(current_node_id);

            // 判断当前节点是否是地面站（你这里是 1156 和 1157）
            bool is_ground_station = (current_node_id == 1156 || current_node_id == 1157);

            if (is_ground_station) {
                // 地面站：使用多路径设置函数（你已新增 SetMultiPathForwardState）
                arbiter->SetMultiPathForwardState(
                    (int32_t)target_node_id,
                    (int32_t)path_id,
                    (int32_t)next_hop_node_id,
                    1 + (int32_t)my_if_id,
                    1 + (int32_t)next_if_id
                );
            } else {
                // 卫星节点：仍然使用原来的单路径函数（忽略 path_id）
                // 注意：如果文件里给了 path_id，我们只认 path_id=0 的，或者任意一个（因为卫星只有一条）
                // if (path_id == 0) {  // 推荐：只在 path_id=0 时设置，避免重复覆盖
                    arbiter->SetSingleForwardState(
                        (int32_t)target_node_id,
                        (int32_t)next_hop_node_id,
                        1 + (int32_t)my_if_id,
                        1 + (int32_t)next_if_id
