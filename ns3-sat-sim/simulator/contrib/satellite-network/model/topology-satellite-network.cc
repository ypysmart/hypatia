...
namespace ns3 {
    ...
    TopologySatelliteNetwork::CreateGSLs() {
        ...
        gsl_helper.SetDeviceAttribute ("DataRate", DataRateValue (DataRate (std::to_string(m_gsl_data_rate_megabit_per_s) + "Mbps")));//星地链路贷款
        ...
        if (fstate_file) {
            ...
            while (getline(fstate_file, line)) {
                ...
                // std::cout << "  >> GS" << line_counter-1 << ": " << num_ifs << " GSLs/////////////////////////ypy" << std::endl; 
