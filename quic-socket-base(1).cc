...
QuicSocketBase::Bind (const Address &address)
{
  ...
  if (InetSocketAddress::IsMatchingType (address))
    {
      ...
      if (ipv4 == Ipv4Address::GetAny () && port == 0)
        {
          m_endPoint = m_quicl4->Allocate ();//分配端点。如果失败，设置错误码。
        }
  ...
  m_quicl4->UdpBind (address, this);//绑定UDP（QUIC基于UDP）
  return SetupCallback ();//设置回调函数
}

int
QuicSocketBase::Bind6 (void)//IPv6版本的Bind
{
  //NS_LOG_FUNCTION (this);
  m_endPoint6 = m_quicl4->Allocate6 ();
  if (0 == m_endPoint6)
  ...
  if (InetSocketAddress::IsMatchingType (address))
    {
      ...
      m_endPoint6 = nullptr;
//创建子流：m_pathManager->InitialSubflow0(...)（多路径初始子流）
...
QuicSocketBase::AppendingTx (Ptr<Packet> frame)
{
  ...
  if (m_socketState != IDLE)
    {
      bool done = m_txBuffer->Add (frame);//添加到缓冲
      if (!done)
        {
      ...
      if (m_socketState != IDLE)
        {
          if (!m_sendPendingDataEvent.IsRunning ())//调度发送：（用模拟器调度发送）。
            {
...
//发送缓冲中的数据，关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！关键！
uint32_t
QuicSocketBase::SendPendingData (bool withAck)
{
  ...
  std::vector<double> sendP = m_scheduler->GetNextPathIdToUse();//用调度器m_scheduler->GetNextPathIdToUse()决定用哪个路径发送。
...
//检查步调定时器、窗口大小等，调用SendDataPacket发送。
...
//这是发送一个数据包的核心方法。它从发送缓冲区取出数据，构建包头部，处理步调（pacing），发送包，并更新状态。像“邮寄包裹”：打包数据、加标签、检查是否能发。
uint32_t
...
//设置重传超时定时器（alarm）。如果包丢失，定时器到期后触发重传。像“闹钟”：提醒检查包是否丢失。
void
...
//多路径方法：void QuicSocketBase::CreateNewSubflows()：创建新子流。
void 
