...
namespace ns3 {
    ...
    private:
    int m_numInterfacesPerGs = 3;  // **每个地面站接口数**（2=双路径）
    int m_numInterfacesPerSat = 4; // **每个卫星接口数**
    // 在类 TopologySatelliteNetwork 的 private 部分添加：
    //多路径支持
        // std::vector<std::vector<Ipv4Address>> m_nodeInterfaces;  // 每个节点的多个 IP 接口
        // int m_numInterfacesPerNode;  // 每个节点接口数（例如 2 或 3，根据你的网络）
        // Build functions
        void ReadConfig();
