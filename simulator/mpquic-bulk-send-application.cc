...
// 如果存在 m_unsentPacket（来自先前的部分发送），则重用它。

// 如果 EnableSeqTsSizeHeader：添加 SeqTsSizeHeader（seq++, size=toSend），创建（toSend - 头部大小）的负载数据包，进行跟踪，然后添加头部。

// 否则：简单的 Create<Packet>(toSend)——空负载模拟数据。
      Ptr<Packet> packet;
      if (m_unsentPacket)
        {
...
//通过 m_socket->Send(packet, 1) 发送：标志 1 可能表示 QUIC 特定处理（例如，流 ID）。
      int actual = m_socket->Send (packet,1);
      if ((unsigned) actual == toSend)
        {
          NS_LOG_LOGIC("Send success: " << actual << " bytes sent, total: " << m_totBytes << " at time " << Simulator::Now().GetSeconds() << "s");
          m_totBytes += actual;
          m_txTrace (packet);
          m_unsentPacket = 0;
        }
      else if (actual == -1)
        {
          ...
          NS_LOG_DEBUG("Send failed (buffer full): attempted " << toSend << " bytes, caching packet");
      ...
      else if (actual > 0 && (unsigned) actual < toSend)
        {
          ...
          NS_LOG_DEBUG("Partial send: " << actual << " bytes sent out of " << toSend << ", fragmenting packet");
