                                    ...
                                    Ptr<QuicSocketTxItem> ackedPacket, double alpha, double sum_rate)
{
  ...
  if (InRecovery (tcb, ackedPacket->m_packetNumber))
    {
      std::cout<<"   In  recovery，，tcbd->m_cWnd:"<<tcbd->m_cWnd<<std::endl;
  ...
  if (tcbd->m_cWnd < tcbd->m_ssThresh)
    {
      ...
      std::cout<<this<<"   In slow start，，tcbd->m_cWnd:"<<tcbd->m_cWnd<<std::endl;
    }
  else
    {
      ...
      if (tcbd->m_cWnd > (uint32_t) 0) {
        ...
        std::cout<<this<<"   In Congestion Avoidance tcbd->m_cWnd:"<<tcbd->m_cWnd<<std::endl;
      } else {
        tcbd->m_cWnd = tcbd->m_kMinimumWindow;
        std::cout<<this<<"  In congestion avoidance,,被设置为tcbd->m_kMinimumWindow;"<<tcbd->m_kMinimumWindow<<std::endl;
      }
    }
}
