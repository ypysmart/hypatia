...
def algorithm_free_gs_many_sat_many_only_over_isls(
    ...
    fstate = calculate_fstate_shortest_path_without_gs_relaying_multi_if(  # 使用新函数，见步骤3
        output_dynamic_state_dir,
        time_since_epoch_ns,
        len(satellites),
        len(ground_stations),
        sat_net_graph_only_satellites_with_isls,
        num_isls_per_sat,
        gid_to_connections,  # 修改为connections dict
        ground_station_satellites_in_range_selected,
        sat_neighbor_to_if,
        prev_fstate,
        enable_verbose_logs
    )
